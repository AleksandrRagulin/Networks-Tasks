...
int main(int argc, char const *argv[])
{
    ...
    while (run)
    {
        ...
        {
            buffer[recv_len] = '\0';
            client_addr = inet_ntop(AF_INET, &client_address.sin_addr, client_address_buf, sizeof(client_address_buf) / sizeof(client_address_buf[0]));
            std::cout
                << "Client with address "
                << client_addr
	...
	// Save client command into variable command_string           
	...
	// Check if command is "exit". 
            ...
            {
		run = false;
		std::cout << "Server closes" << std::endl;
		}
